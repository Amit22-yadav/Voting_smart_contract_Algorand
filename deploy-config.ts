...
export async function deploy(name: (typeof contracts)[number], appSpec: AppSpec) {
  ...
  const deployer = await algosdk.mnemonicToSecretKey("exact absorb frozen make monkey note boat library move uncle safe tell asthma alter cousin damp rural charge vessel repeat ask ghost alone above believe");
  console.log("heyy==== ",deployer)
  await algokit.ensureFunded(
    {
      accountToFund: deployer,
      //fundingSource: await algokit.getDispenserAccount(algod),
    //ßß  fundingSource:deployer.addr,
